            ...
            try {
                ...
                switch (clickedButton.getId()) {
                    ...
                    case "btnActivate":
                        ...
                        String id = oParameters.Branch().getModel().getBranchCode();
                        JSONObject poJsON;
                        
                        switch (Status) {
                            case "0":
                                if (ShowMessageFX.YesNo(null, pxeModuleName, "Do you want to Activate this Parameter?") == true) {
                                    ShowMessageFX.Information(String.valueOf(oParameters.Branch().getEditMode()), "Computerized Accounting System", pxeModuleName);
                                    oParameters.Branch().initialize();
                                    poJsON = oParameters.Branch().activateRecord();
                                    if ("error".equals(poJsON.get("result"))) {
                                        ShowMessageFX.Information((String) poJsON.get("message"), "Computerized Accounting System", pxeModuleName);
                                        break;
                                    }
                                    poJsON = oParameters.Branch().openRecord(id);
                                    if ("error".equals(poJsON.get("result"))) {
                                        ShowMessageFX.Information((String) poJsON.get("message"), "Computerized Accounting System", pxeModuleName);
                                        break;
                                    }
                            ...
                            case "1":
                                if (ShowMessageFX.YesNo(null, pxeModuleName, "Do you want to Deactivate this Parameter?") == true) {
                                    ...
                                    System.out.println("EDIT MODE : " + oParameters.Branch().getEditMode());
                                    ShowMessageFX.Information(String.valueOf(oParameters.Branch().getEditMode()), "Computerized Accounting System", pxeModuleName);
