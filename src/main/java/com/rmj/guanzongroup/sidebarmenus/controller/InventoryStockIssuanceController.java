...
public class InventoryStockIssuanceController implements Initializable, ScreenInterface {
    ...
    @FXML
    private Button btnBrowse, btnSearch, btnUpdate,
            btnPrint, btnVoid, btnSave,
            btnCancel, btnHistory, btnRetrieve, btnClose,
            btnSaveDelelivery, btnPrintDelivery, btnCancelDelivery, btnNew;
    ...
    private TableColumn<Model_Inventory_Transfer_Detail, String> tblColDetailNo, tblColDetailOrderNo, tblColDetailSerial,
            ...
            tblColDetailBrand, tblColDetailVariant, tblColDetailCost,
...
//            tfPlateNo.setText(poAppController.getMaster().getRemarks());
    ...
    private void loadSelectedTransactionDetail(int fnRow) throws SQLException, GuanzonException, CloneNotSupportedException {
        ...
        dpDeliveryDate.setValue(ParseDate(poAppController.getDetail(fnRow).InventoryTransferMaster().getTransactionDate()));
        taDeliveryRemarks.setText(poAppController.getDetail(fnRow).InventoryTransferMaster().getRemarks());
    ...
    private void loadSelectedTransactionDetailOther(int fnRow) throws SQLException, GuanzonException, CloneNotSupportedException {
        ...
        tfSupersede.setText(poAppController.getDetailOther(pnTransactionDetail)
                .getDetail(fnRow).InventorySupersede().getBarCode());
        tfModel.setText(poAppController.getDetailOther(pnTransactionDetail)
                .getDetail(fnRow).Inventory().Model().getDescription());
        tfColor.setText(poAppController.getDetailOther(pnTransactionDetail)
                .getDetail(fnRow).Inventory().Color().getDescription());
        tfMeasure.setText(poAppController.getDetailOther(pnTransactionDetail)
                .getDetail(fnRow).Inventory().Measure().getDescription());
        tfInvType.setText(poAppController.getDetailOther(pnTransactionDetail)
    ...
    private void initializeTableDetailOther() {
        if (laTransactionDetailOther == null) {
            ...
            tblColDetailApprovedQty.setCellValueFactory((loModel) -> {
                try {
                    return new SimpleStringProperty(String.valueOf(loModel.getValue().InventoryStockRequest().getApproved()));
                } catch (SQLException | GuanzonException e) {
                    poLogWrapper.severe(psFormName, e.getMessage());
                    return new SimpleStringProperty("");
                }
    ...
    private void reloadTableDetailOther()
        ...
        List<Model_Inventory_Transfer_Detail> rawDetail = poAppController.getDetailOther(pnTransactionDetailOther).getDetailList();
    ...
    private void initializeTableDetail() {
        if (laTransactionDetail == null) {
            ...
            tblColDelTransNo.setCellValueFactory((loModel) -> new SimpleStringProperty(loModel.getValue().getTransactionNo()));
            ...
            tblColDelDate.setCellValueFactory((loModel) -> {
                try {
                    return new SimpleStringProperty(SQLUtil.dateFormat(
                            loModel.getValue().InventoryTransferMaster().getTransactionDate(), SQLUtil.FORMAT_LONG_DATE));
            ...
            tblColDelTransNo.setCellValueFactory((loModel) -> new SimpleStringProperty(String.valueOf(loModel.getValue().getNoOfItem())));

            tblColDelStatus.setCellValueFactory((loModel) -> {
                try {
                    return new SimpleStringProperty(
                            InventoryStockIssuanceStatus.STATUS.get(
                                    Integer.parseInt(loModel.getValue().InventoryTransferMaster().getTransactionStatus())));
                } catch (SQLException | GuanzonException e) {
                    poLogWrapper.severe(psFormName, e.getMessage());
                    return new SimpleStringProperty("");
                }
